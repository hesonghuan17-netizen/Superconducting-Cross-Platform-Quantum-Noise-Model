...
def apply_2qg2(c, index1, index2, p1, p, lut, px_py_pz):
    ...
    c.append("H", index1)
    c.append("PAULI_CHANNEL_1", index1, px_py_pz[index1])
    c.append("DEPOLARIZE1", index1, p1[index1])
...
def generate_prefix(num_qubits, p1, p, lut, px_py_pz):
    ...
    # Add H gates
    script += "H " + " ".join(str(i) for i in range(0, num_qubits, 2)) + "\n"

    for i in range(0, num_qubits, 2):
        script += f"PAULI_CHANNEL_1({px_py_pz[i][0]}, {px_py_pz[i][1]}, {px_py_pz[i][2]}) {i}\n"
        script += f"DEPOLARIZE1({p1[i]}) {i}\n"

    # Add H gates for all qubits
    script += "H " + " ".join(str(i) for i in range(0, num_qubits)) + "\n"
...
def generate_circuits(SHOTS, _rounds_ignored, num_qubits, lp, sp, spam_rates, spam_rates_initial,  sqg_fid, ecr_fid, t1_t2_values, ecr_lengths,
    ...
    c.append("H", 0)
    c.append("PAULI_CHANNEL_1", 0, px_py_pz[0])
    c.append("DEPOLARIZE1", 0, p1[0])

    for i in range(0, num_qubits, 2):
        c.append("H", i)
        c.append("PAULI_CHANNEL_1", i, px_py_pz[i])
        c.append("DEPOLARIZE1", i, p1[i])
